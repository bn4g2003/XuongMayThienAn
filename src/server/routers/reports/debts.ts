...
const getDebtReportSchema = z.object({
  type: z.enum(['list', 'summary']).default('list'),
  dateRange: z.object({
  ...
  debtType: z.string().optional(), // comma-separated values
...
const debtsRouter = router({
  getReport: protectedProcedure
    .input(getDebtReportSchema)
    .query(async ({ input }) => {
      // Permission check
      const { hasPermission, error } = await requirePermission('finance.debts', 'view');
      if (!hasPermission) {
        throw new Error(error || 'Không có quyền xem báo cáo công nợ');
      }
      ...
      if (dateRange?.from || dateRange?.to) {
        ...
        if (dateRange.from) {
          whereConditions.created_at.gte = new Date(dateRange.from);
        }
        if (dateRange.to) {
          whereConditions.created_at.lte = new Date(dateRange.to);
        }
      }
      ...
      if (branchId) {
        ...
        if (debtTypes.length === 1 && debtTypes[0] === 'CUSTOMER') {
          whereConditions.customers = { branch_id: branchId };
        } else if (debtTypes.length === 1 && debtTypes[0] === 'SUPPLIER') {
          whereConditions.suppliers = { branch_id: branchId };
        } else {
          // Multiple debt types or no specific type, check both
          const branchCondition = {
            OR: [
              { customers: { branch_id: branchId } },
