...
export const getUserPermissions = async (
  ...
  // Fetch role_permissions and include permission metadata
  const rps = await db.role_permissions.findMany({
    where: { role_id: roleId },
    include: { permissions: true },
  }) as any[];

  console.log(`[getUserPermissions] Role ${roleId} has ${rps.length} permissions`);

  // Map to Permission[] and sort by module then permission_code to match original order
  const moduleByCode = new Map<string, string>();
  rps.forEach((rp: any) => {
    if (rp.permissions) moduleByCode.set(rp.permissions.permission_code, rp.permissions.module || '');
  });
  ...
  const mapped = rps
    ...
    .filter((x: Permission | null): x is Permission => x !== null)
    .sort((a: Permission, b: Permission) => {
      const pa = moduleByCode.get(a.permissionCode) || '';
      const pb = moduleByCode.get(b.permissionCode) || '';
      if (pa === pb) return a.permissionCode.localeCompare(b.permissionCode);
...
> => {
  const perms = await db.permissions.findMany({
    orderBy: [{ module: 'asc' }, { permission_code: 'asc' }],
    select: {
      id: true,
