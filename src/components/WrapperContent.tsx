  ...
  return (
    <div className={`space-y-10 ${className}`}>
      ...
      {!isMobileView && header.filters && header.filters.fields && (
        <div className="mt-4">
          <FilterList
            initValues={header.filters.query}
            form={formFilter}
      ...
      {!isLoading && !isNotAccessible && !isEmpty && (
        <div className="relative">
          {children}
          {isRefetching && (
            <div className="absolute inset-0 bg-background opacity-45 border-transparent rounded-2xl flex items-center justify-center z-10">
              <LoaderApp />
            </div>
          )}
        </div>
