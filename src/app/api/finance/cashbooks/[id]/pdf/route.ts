...
export async function GET(
  ...
  try {
    ...
    const cashbook = {
      id: cashbookData.id,
      transactionCode: cashbookData.transaction_code,
      transactionDate: cashbookData.transaction_date,
      amount: Number(cashbookData.amount),
      transactionType: cashbookData.transaction_type,
      paymentMethod: cashbookData.payment_method,
      description: cashbookData.description,
      categoryName: cashbookData.financial_categories?.category_name,
      categoryCode: cashbookData.financial_categories?.category_code,
      bankAccountNumber: cashbookData.bank_accounts?.account_number,
      bankName: cashbookData.bank_accounts?.bank_name,
      bankBranchName: cashbookData.bank_accounts?.branch_name,
      createdBy: cashbookData.users?.full_name,
      branchName: cashbookData.branches?.branch_name,
      branchAddress: cashbookData.branches?.address,
      branchPhone: cashbookData.branches?.phone,
      createdAt: cashbookData.created_at
    };

    // Lấy thông tin công ty
    const company = (await db.company_config.findFirst())
...
<body>
  ...
  <div class="content">
    ...
    <div class="amount-row">
      ...
      <div class="amount-value">${parseInt(cashbook.amount.toString()).toLocaleString('vi-VN')} đồng</div>
  ...
  <script>
    ...
    window.onload = function() {
      const amountWords = numberToVietnameseWords(${parseInt(cashbook.amount.toString())});
