...
export async function GET(
  ...
  try {
    ...
    const poData = await db.purchase_orders.findUnique({
      where: { id: poId },
      include: {
        suppliers: true,
        users: true,
    ...
    const po = {
      ...
      supplierName: poData.suppliers?.supplier_name,
      supplierPhone: poData.suppliers?.phone,
      supplierAddress: poData.suppliers?.address,
      orderDate: poData.order_date,
      expectedDate: poData.expected_date,
      totalAmount: Number(poData.total_amount),
      status: poData.status,
      notes: poData.notes,
      createdBy: poData.users?.full_name,
      createdAt: poData.created_at,
      branchName: poData.branches?.branch_name,
      branchAddress: poData.branches?.address,
      branchPhone: poData.branches?.phone
    };

    // Lấy chi tiết
    const detailsRaw = await db.purchase_order_details.findMany({
    ...
    const formattedDetails = detailsRaw.map(detail => ({
      id: detail.id,
      itemCode: detail.item_code || detail.materials?.material_code,
      itemName: detail.item_name || detail.materials?.material_name,
    ...
    })) as { company_name?: string; tax_code?: string; address?: string; phone?: string; email?: string } | null || {};
...
<body>
  ...
  <div class="info-section">
    ...
    <div class="info-row">
      ...
      <div class="info-value">
        <span class="status-badge status-${(po.status || 'UNKNOWN').toLowerCase()}">${statusMap[po.status || 'UNKNOWN'] || po.status || 'UNKNOWN'}</span>
      </div>
    </div>
  </div>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="text-right">${item.quantity.toLocaleString('vi-VN')}</td>
        <td class="text-right">${item.unitPrice.toLocaleString('vi-VN')}</td>
        <td class="text-right"><strong>${item.totalAmount.toLocaleString('vi-VN')}</strong></td>
      </tr>
      `).join('')}
    </tbody>
  </table>
  ...
  <div class="total-section">
    <div class="total-row final">
      <span>TỔNG TIỀN:</span>
      <span>${po.totalAmount.toLocaleString('vi-VN')} đ</span>
    </div>
  </div>
