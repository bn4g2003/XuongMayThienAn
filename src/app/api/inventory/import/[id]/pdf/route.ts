...
export async function GET(
  ...
  try {
    ...
    const transaction = {
      ...
      warehouseName: trans.warehouses_inventory_transactions_to_warehouse_idTowarehouses?.warehouse_name,
      branchName: trans.warehouses_inventory_transactions_to_warehouse_idTowarehouses?.branches?.branch_name,
      branchAddress: trans.warehouses_inventory_transactions_to_warehouse_idTowarehouses?.branches?.address,
      notes: trans.notes,
    ...
    const html = `
      ...
      <body>
        ...
        <div class="total">
          <p>Tổng cộng: ${detailsResult.reduce((sum, item) => sum + (item.totalAmount || 0), 0).toLocaleString()} đ</p>
        </div>
