...
export async function GET(
  ...
  try {
    ...
    const order = {
      ...
      customerPhone: orderData.customers?.phone,
      customerAddress: orderData.customers?.address,
      ...
      branchName: orderData.branches?.branch_name,
      branchAddress: orderData.branches?.address,
      branchPhone: orderData.branches?.phone
    };
...
<body>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="text-right">${Math.floor(item.unitPrice).toLocaleString('vi-VN')}</td>
        <td class="text-right"><strong>${Math.floor(item.totalAmount).toLocaleString('vi-VN')}</strong></td>
      </tr>
      `).join('')}
    </tbody>
  </table>
  ...
  <div class="total-section">
    <div class="total-row">
      <span>Tổng tiền:</span>
      <span>${Math.floor(order.totalAmount).toLocaleString('vi-VN')} đ</span>
    </div>
    ${order.discountAmount > 0 ? `
    <div class="total-row" style="color: #dc2626;">
      <span>Giảm giá:</span>
      <span>-${Math.floor(order.discountAmount).toLocaleString('vi-VN')} đ</span>
    </div>` : ''}
    <div class="total-row final">
      <span>THÀNH TIỀN:</span>
      <span>${Math.floor(order.finalAmount).toLocaleString('vi-VN')} đ</span>
    </div>
  </div>
